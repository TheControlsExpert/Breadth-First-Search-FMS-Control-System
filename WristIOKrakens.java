...
public class WristIOKrakens implements WristIO {
   ...
   // private DigitalInput limitSwitch = new DigitalInput(0);
    ...
    ProfiledPIDController profiler_PID = new ProfiledPIDController(0.005, 0, 0, new Constraints(300 * 10/60, 200 * 6/60));
    DoubleProfilerArmNeo profiler_new = new DoubleProfilerArmNeo(profiler_PID, pivotMotor);
    ...
    public void updateInputs(WristIOInputs inputs, ManualMode mode) {
        ...
        double setpoint = profiler_PID.calculate(pivotMotor.getEncoder().getPosition(), setpointangle);
        double setpoint2 = profiler_new.calculate(setpointangle);
   
    SmartDashboard.putNumber("setpoint",  profiler_PID.getSetpoint().velocity * 60 );
   ...
   // SmartDashboard.putNumber("velocity pivot",  pivot.getEncoder().getVelocity());
       ...
       //  motorL.set(0.05);
        pivotMotor.getClosedLoopController().setReference(profiler_PID.getSetpoint().velocity * 60, ControlType.kVelocity, ClosedLoopSlot.kSlot0, 12 * (Math.signum(profiler_PID.getSetpoint().velocity) * ks)  +  12 * (kg * Math.cos(Units.rotationsToRadians(armEncoder.get() - offset))) + kv * profiler_PID.getSetpoint().velocity * 60 + setpoint, ArbFFUnits.kVoltage);
        }
